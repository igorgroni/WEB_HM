...
def prepare_data(conn, NUMBER_STUDENTS, NUMBER_TEACHERS, NUMBER_GROUP, lessons):
   ...
   # Додаємо студентів до таблиці students
    for student_name in students:
        cur.execute("INSERT INTO students (FirstName, LastName, GroupID) VALUES (?, ?, ?)",
                    (student_name.split()[0], student_name.split()[1], choice(range(1, NUMBER_GROUP + 1))))
    ...
    for teacher_name in teachers:
        cur.execute("INSERT INTO teachers (FirstName, LastName) VALUES (?, ?)",
                    (teacher_name.split()[0], teacher_name.split()[1]))

    # Додаємо уроки до таблиці lessons
    for lesson_name in lessons:
        cur.execute("INSERT INTO lessons (LessonName, TeacherID) VALUES (?, ?)",
    ...
    for student_id in range(1, NUMBER_STUDENTS + 1):
        for lesson_id in range(1, len(lessons) + 1):
            ...
            for grade in grades:
                cur.execute("INSERT INTO rating (StudentID, LessonID, Rate, ExamDate) VALUES (?, ?, ?, ?)",
